...
{
  static_assert(!std::is_pointer<T>::value, ""); // Protection against pointer reading
  storage.read(blockIndex.absoluteAddress(), sizeof(T), reinterpret_cast<char *>(&obj));
}
...
{
  static_assert(!std::is_pointer<T>::value, ""); // Protection against pointer writing
  storage.write(blockIndex.absoluteAddress(), sizeof(T), reinterpret_cast<char const *>(&obj));
}
